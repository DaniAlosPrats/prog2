...
foreach($data as $estacion){
	$latitud=explode(",",$estacion["coordenadas"])[0];
	$temperatur=$estacion["numberrange"];
	if($latitud >0){
		$estacionnorte++;
	} elseif ($latitud<0) {
		$estacionsur[]=$temperatur;
	} if($temperatur<-20 && $temperatur>40){
		$tempretaruaext[]=$estacion["estacion"];
	}
}
$temperatura_media_sur = array_sum($temperaturas_sur) / count($temperaturas_sur);
echo"los de emisferio norte son $estacionnorte";
echo"la media de emisferio sur es  $temperatura_media_sur";
echo"los menos de -20  y mas de 40 son  $tempretaruaext";
?>